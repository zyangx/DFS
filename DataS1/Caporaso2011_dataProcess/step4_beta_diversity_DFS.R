...
### read in raw data
otu_table.m <- as.matrix(read.csv("otu_table_filter100_rare_filter.txt", header=T, row.names=1, sep="\t", skip=1, check.names=FALSE))
...
write.table(DFS.m, "DFS_otu_table_filter100_rare_filter.txt", sep="\t", quote=F, col.names = NA)
